...
program
  ...
  .option(
    "-o, --output <file>",
    "Output PDF file (for single file) or directory (for batch)",
  )
...
async function handleDirectory(inputPath, options) {
  ...
  for (const filePath of markdownFiles) {
    try {
      ...
      console.error(
        chalk.red(
          `❌ Failed to convert ${basename(filePath)}: ${error.message}`,
        ),
      );
    }
  }
  ...
  console.log(
    chalk.green(
      `✅ Converted ${successCount}/${markdownFiles.length} files successfully!`,
    ),
  );
}
